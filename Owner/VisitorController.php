...
class VisitorController{
    ...
    public function ApplyVisitor($name, $IC, $car_number_plate, $email, $phone, $visitor_code, $visit_date, $status, $owner_id, $valid_days){
        // 设置默认时区
        ...
        if ($count > 0) {
            ...
            // 插入访客数据
            $stmt = $this->conn->prepare("INSERT INTO visitors (name, IC, car_number_plate, email, phone, visitor_code, qr_code, visit_date, status, owner_id, valid_days) VALUES (?, ?,?, ?, ?, ?, ?, ?, ?, ?,?)");
            $stmt->bind_param("sssssssssss", $name, $IC, $car_number_plate, $email, $phone, $visitor_code, $filePath, $visit_date, $status, $owner_id, $valid_days);
            $stmt->execute();
